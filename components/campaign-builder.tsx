...
export default function CampaignBuilder({ mode, campaignId }: CampaignBuilderProps) {
  ...
  const [linkButtonLabel, setLinkButtonLabel] = useState(t("Open link"));
  ...
  useEffect(() => {
    ...
    fetch("/api/automations", { cache: "no-store" })
      ...
      .then((payload) => {
        ...
        setLinkButtonLabel(c.linkButtonLabel ?? t("Open link"));
        ...
        setSecondaryButtonLabel(secondLink?.label ?? t("Open link"));
        ...
        setFollowPromptButtonLabel(
          c.followPromptButtonLabel ?? t("i'm following")
        );
  ...
  function prefillFromRow(row: ImportRow) {
    ...
    setOpeningDmButtonLabel(
      row.openingDmButtonLabel || (hasOpening ? t("Send link") : "")
    );
  ...
  async function handleSubmit(activeValue: boolean) {
    ...
    if (!selectedAccountId)
      return setError(t("Connect an Instagram account first."));
    ...
    const payload = {
      ...
      linkButtonLabel: linkButtonLabel.trim() || t("Open link"),
      secondaryDestinationUrl: secondaryDestinationUrl.trim() || "",
      secondaryButtonLabel: secondaryButtonLabel.trim() || t("Open link"),
      requireFollow,
      followPromptMessage: requireFollow ? followPromptMessage.trim() : "",
      followPromptButtonLabel: requireFollow
        ? followPromptButtonLabel.trim() || t("i'm following")
        : "",
      followUpEnabled,
      ...
      <div className="space-y-8 min-w-0">
        ...
        <Section title={t("And this comment has")}>
          ...
          <div className="flex items-center justify-between gap-3 rounded-lg border border-border px-3 py-2.5">
            <span className="text-sm text-foreground">
              {matchMode === "any"
                ? t("also reply when someone DMs anything")
                : t("also reply when someone DMs these words")}
            </span>
            <Toggle
              on={dmTriggerEnabled}
        ...
        <Section title={t("And then, they will get")}>
          <div className="rounded-lg border border-border p-3 space-y-2">
            ...
            <p className="text-xs text-muted">
              {t("{link} inserts the tracked link; {username} personalizes.")}
            </p>
          </div>
          <div className="mt-3 rounded-lg border border-border p-3">
            <div className="flex items-center justify-between">
              ...
              <div className="mt-3 space-y-2">
                ...
                <p className="text-xs text-muted">
                  {followUpDelayMinutes > 0
                    ? t("Sent {minutes} min after they tap through.", {
                        minutes: followUpDelayMinutes,
                      })
      ...
      <div>
        ...
        <div className="flex min-w-0 justify-center lg:sticky lg:top-6 lg:block">
          <CampaignPreview
            ...
            linkButtonLabel={linkButtonLabel || t("Open link")}
            linkUrl={trackedDestinationUrl.trim() || undefined}
            hasSecondLink={
              secondLinkOpen && Boolean(secondaryDestinationUrl.trim())
            }
            secondLinkButtonLabel={secondaryButtonLabel || t("Open link")}
